...
from STYLEGAN2.utils.utils_stylegan2 import get_weight_initializer_runtime_coef
from STYLEGAN2.dnnlib.ops.upfirdn_2d import upsample_conv_2d, conv_downsample_2d
...
class ModulatedConv2DLayer(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        self.init_std_w, self.runtime_coef_w = get_weight_initializer_runtime_coef(
            shape=[self.kernel, self.kernel, input_shape[1], self.fmaps],
            gain=self.gain, use_wscale=self.use_wscale, lrmul=self.lrmul)

        self.init_std_s, self.runtime_coef_s = get_weight_initializer_runtime_coef(
            shape=[self.latent_size, input_shape[1]],
            gain=self.gain, use_wscale=self.use_wscale, lrmul=self.lrmul)
    ...
    def call(self, x, dlatent_vect):
        ...
        # Modulate.
        s = tf.matmul(dlatent_vect,
                      tf.math.multiply(self.mod_weight, self.runtime_coef_s))  # [BI] Transform incoming W to style.
        s = tf.nn.bias_add(s, tf.math.multiply(self.mod_bias, self.lrmul)) + 1  # [BI] Add bias (initially 1).

        ww = tf.math.multiply(ww, tf.cast(s[:, np.newaxis, np.newaxis, :, np.newaxis],
        ...
        if self.fused_modconv:
            ...
            x = tf.math.multiply(x, tf.cast(s[:, :, np.newaxis, np.newaxis],
        ...
        if self.up:
            ...
            if self.gpu:
                x = conv_downsample_2d(x, tf.cast(w, x.dtype), data_format='NCHW', k=self.resample_kernel,
                                       impl=self.impl)
            else:
                x = tf.transpose(x, [0, 2, 3, 1])
                x = conv_downsample_2d(x, tf.cast(w, x.dtype), data_format='NHWC', k=self.resample_kernel,
                                       impl=self.impl, gpu=False)
                x = tf.transpose(x, [0, 3, 1, 2])
        else:
            if self.gpu:
        ...
        if self.fused_modconv:
            x = tf.reshape(x, [-1, self.fmaps, x.shape[2],
                               x.shape[3]])  # Fused => reshape convolution groups back to minibatch.
        elif self.demodulate:
            x = tf.math.multiply(x, tf.cast(d[:, :, np.newaxis, np.newaxis],
