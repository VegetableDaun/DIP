...
class BlockLayer(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        self.conv2d_0 = Conv2DLayer(fmaps=nf(self.res-1), kernel=3, name='Conv0')
        ...
        self.conv2d_1_down = Conv2DLayer(fmaps=nf(self.res-2), kernel=3, down=True, name='Conv1_down')
        
        self.bias_1_down = self.add_weight(name='Conv1_down/bias', shape=(nf(self.res-2),), 
                                           initializer=tf.random_normal_initializer(0, 1), trainable=True)
        
        self.conv2d_skip = Conv2DLayer(fmaps=nf(self.res-2), kernel=1, down=True, name='Skip')
    ...
    def call(self, x):
        ...
        a = tf.reshape(self.bias_1_down, [-1 if i == 1 else 1 for i in range(x.shape.rank)])
        x += a
        x = tf.math.multiply(tf.nn.leaky_relu(x, 0.2), tf.math.sqrt(2.))
